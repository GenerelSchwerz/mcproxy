...
type Registry = ReturnType<typeof import("prismarine-registry")>;
...
export function generateLoginPacket(stateData: StateData, pclient?: Client): Packet {
  const version = stateData.bot.version;
  const mcVer = stateData.bot.registry.version;

  // if (stateData.bot.registry.loginPacket) {
  //   return ["login", stateData.bot.registry.loginPacket];
  // }

  if (stateData.rawLoginPacket) {
    return ["login", stateData.rawLoginPacket];
  }

  const packet = require(`../data/${version}.login.json`);
  if (!stateData.rawLoginPacket) {
  ...
  const newPacket: { [key: string]: any } = {
    entityId: stateData.bot.entity.id,
    isHardcore: stateData.bot.game.hardcore,
    gameMode: stateData.bot.player.gamemode,
    previousGameMode: -1,
    worldNames: stateData.rawLoginPacket.worldNames ?? ["minecraft:overworld", "minecraft:the_nether", "minecraft:the_end"],
  ...
  // TODO: version specific schenanigans
  if (mcVer["=="]("1.19.4")) {
    if (!stateData.rawLoginPacket?.dimensionCodec) {
      if (!(stateData.bot.registry as any).registryCodec) {
        throw new Error("dimensionCodec not found in rawLoginPacket");
      }
      newPacket["dimensionCodec"] = !(stateData.bot.registry as any).registryCodec; // this is a thing.
    } else {
      newPacket["dimensionCodec"] = packet.dimensionCodec;
    }
  }
  return ["login", newPacket];
}
...
export function* generatePackets(
  ...
  const refData = stateData.bot.registry;
  ...
  // yield ['login', bot.registry.loginPacket];
  ...
  // unneeded to spawn
  // Updating held item
  yield ["held_item_slot", { slot: bot.quickBarSlot ?? 0 }];

  // unneeded to spawn
  // declare recipes (requires prismarine-registry)
  if (stateData.rawRecipes) yield ["declare_recipes", stateData.rawRecipes];

  // unneeded to spawn
  // load "tags", whatever that is
  // if (stateData.rawTags) yield ["tags", stateData.rawTags];
  ...
  if (stateData.rawCommandPacket) yield ["declare_commands", stateData.rawCommandPacket];

  // unneeded to spawn
  // unlock recipes
  if (stateData.rawUnlockRecipes) yield ["unlock_recipes", stateData.rawUnlockRecipes];
  ...
  if (refData.supportFeature("signedChat")) {
    yield [
      "server_data",
      {
        motd: { type: "string", value: "lmao placeholder" }, //'{"text":"lmao placeholder"}',
  ...
  for (const val of convertPlayers(refData, bot.players, UUID)) {
    yield val;
  }

  // 1.21.1
  // world border data needed here
  if (stateData.rawWorldBorder) yield ["initialize_world_border", stateData.rawWorldBorder];

  // unneeded to spawn
  // set time to remote bot's
  // NOTE: 1.12.2 requires not bigInt, 1.20 does.
  if (refData.isNewerOrEqualTo("1.13")) yield ["update_time", { age: bot.time.bigAge, time: bot.time.bigTime }];
  ...
  if (bot.isRaining) yield ["game_state_change", { reason: 1, gameMode: 0 }];
  ...
  yield ["chunk_batch_start", {}];
  const worldChunks = convertWorld(bot.world);
  for (const val of worldChunks) {
    yield val;
  }
...
const convertPlayers = (version: Registry, players: Record<string, Player>, UUID: string): Packet[] => {
  const packets: Packet[] = [];
  for (const key in players) {
    const { uuid, username, gamemode, ping, entity } = players[key];
    if (version.isOlderThan("1.13")) {
      packets.push([
        "player_info",
        {
          action: 0,
      ...
      if (version.isNewerOrEqualTo("1.21")) {
        packets.push([
          "player_info",
          {
            action: 63,
